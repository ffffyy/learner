...
class DENCLUE(BaseEstimator, ClusterMixin):
    ...
    def __init__(self, h=None, eps=1e-8, min_density=0.0001, metric='euclidean'):
        self.h = h
        self.eps = eps
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        for clust in clusters:
            ...
            cluster_info[num_clusters] = {'instances': clust.nodes(),
                                          ...
                                          'density': max_density,
                                          'complete': complete}
            print(cluster_info[num_clusters].items())
...
if __name__ == "__main__":
        d = DENCLUE(h=None, eps=1e-8, min_density=0.0001, metric='euclidean')
        path = 'iris.txt'
        data = load_data(path)
        data=np.array(data)
        d.fit(data)
